  ...
  if (error || !member) return (
    <div className="min-h-screen flex items-center justify-center px-4 bg-slate-50 dark:bg-slate-900">
      <div className="text-center">
        ...
        <Link href="/team" className="text-blue-500 hover:text-blue-600 dark:text-blue-400 text-sm">← Back to team</Link>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen py-20 px-4 bg-slate-50 dark:bg-slate-900">
      <div className="max-w-2xl mx-auto animate-fade-in">
        <Link href="/team" className="inline-flex items-center gap-2 text-sm text-slate-400 dark:text-slate-500 hover:text-blue-600 dark:hover:text-blue-400 transition-colors mb-8">
        ...
        <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 sm:p-10 shadow-sm">
          <div className="flex flex-col items-center text-center mb-8">
            ...
            <p className="text-slate-500 dark:text-slate-400 mt-1">{member.role}</p>
          </div>
          <div className="mb-8">
            <h2 className="text-sm font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-3">About</h2>
            <p className="text-slate-600 dark:text-slate-300 leading-relaxed">{member.bio}</p>
          </div>
          {member.skills?.length > 0 && (
            <div className="mb-8">
              <h2 className="text-sm font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-3">Skills</h2>
              <div className="flex flex-wrap gap-2">
                {member.skills.map((s) => (
                  <span key={s} className="px-3 py-1.5 bg-blue-50 dark:bg-blue-900/30 border border-blue-100 dark:border-blue-800 rounded-lg text-sm text-blue-600 dark:text-blue-400">{s}</span>
                ))}
              </div>
            </div>
          )}
          {member.socialLinks && Object.values(member.socialLinks).some(Boolean) && (
            <div>
              <h2 className="text-sm font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-3">Connect</h2>
              <div className="flex flex-wrap gap-3">
                {Object.entries(member.socialLinks).map(([k, url]) => url && (
                  <a key={k} href={url} target="_blank" rel="noopener noreferrer"
                    className="px-4 py-2 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg text-sm text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-all">
