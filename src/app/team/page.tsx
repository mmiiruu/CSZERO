...
function displayName(m: { name: string; nickname: string }) {
...
function SkeletonSection() {
  return (
    <div className="mb-16">
      ...
      <div className="skeleton w-full h-28 rounded-xl mb-4" />
...
function SectionDivider({ label, color, id }: { label: string; color: string; id?: string }) {
  return (
    <div className="flex items-center gap-3 mb-8">
      <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: color }} aria-hidden="true" />
      <h2 id={id} className="text-xs font-mono uppercase tracking-widest whitespace-nowrap font-semibold" style={{ color }}>
        {label}
      </h2>
      <div className="flex-1 h-px bg-border" />
    </div>
...
// Dark forced surface — signals top hierarchy. Stands apart from the page
// background in both light (black-on-white) and dark (elevated blue-tint) mode.
...
function LeaderCard({ member }: { member: TeamMember | FallbackMember }) {
  ...
  const label = member.nickname
    ? `ดูโปรไฟล์ของ ${member.nickname} (${member.name})`
    : `ดูโปรไฟล์ของ ${member.name}`;
  return (
    <Link
...
// Dept-tinted surface (color-mix). No side-stripe — that pattern is banned.
// Mid-weight between dark LeaderCard and bare MemberCard.
...
function DeptHeadCard({ member, deptColor }: { member: TeamMember | FallbackMember; deptColor: string }) {
  ...
  return (
    ...
    >
      <div
        className="border border-border rounded-xl p-5 sm:p-6 flex gap-5 items-center hover:border-border-subtle transition-colors duration-200"
        style={{ backgroundColor: `color-mix(in srgb, ${deptColor} 7%, transparent)` }}
      >
        <div
          className="w-14 h-14 sm:w-16 sm:h-16 shrink-0 rounded-lg flex items-center justify-center text-lg font-black text-white motion-safe:group-hover:scale-105 transition-transform duration-200 overflow-hidden"
          style={{ backgroundColor: accentColor, ...DOT_PATTERN }}
        >
          {member.image ? (
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            <span
              className="inline-block text-xs font-mono font-semibold px-2 py-0.5 rounded-full text-white"
          ...
          <p className="text-xs font-mono text-muted uppercase tracking-wider mt-0.5 truncate">{member.role}</p>
          {member.bio && (
            <p className="mt-1 text-xs text-secondary leading-relaxed line-clamp-1">{member.bio}</p>
          )}
        </div>
        <div
          aria-hidden="true"
          className="shrink-0 flex items-center gap-1 text-xs text-muted group-hover:text-blue-500 dark:group-hover:text-blue-400 transition-colors duration-200"
        >
          <span className="hidden sm:inline">{teamConfig.viewProfileLabel}</span>
          <svg className="w-3.5 h-3.5 motion-safe:group-hover:translate-x-0.5 transition-transform duration-200" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
          </svg>
        </div>
      </div>
...
// ─── Member card ──────────────────────────────────────────────────────────────
// Minimal, low visual weight — lets head cards and leader cards dominate.
...
function MemberCard({ member }: { member: TeamMember | FallbackMember }) {
  ...
  return (
    <Link
      ...
      aria-label={label}
    >
      <div className="bg-card border border-border-subtle rounded-xl p-4 text-center hover:bg-hover hover:border-border transition-colors duration-200 h-full">
        <div
          className="w-12 h-12 rounded-full mx-auto mb-2.5 flex items-center justify-center text-base font-black text-white motion-safe:group-hover:scale-110 transition-transform duration-200 overflow-hidden"
          style={{ backgroundColor: accentColor, ...DOT_PATTERN }}
        >
          {member.image ? (
        ...
        <h3 className="text-foreground font-semibold text-sm leading-snug">{displayName(member)}</h3>
        {member.nickname && (
          <p className="text-muted text-xs leading-snug">{member.name}</p>
        )}
        <p className="text-secondary text-xs mt-0.5 line-clamp-1">{member.role}</p>
        <div
          aria-hidden="true"
          className="mt-2.5 flex items-center justify-center gap-1 text-xs text-muted group-hover:text-blue-500 dark:group-hover:text-blue-400 transition-colors duration-200"
        >
          <span>{teamConfig.viewProfileLabel}</span>
          <svg className="w-3 h-3 motion-safe:group-hover:translate-x-0.5 transition-transform duration-200" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
...
export default function TeamPage() {
  ...
  const unassigned = members.filter(
    (m) => !m.department || !DEPARTMENTS.find((d) => d.key === m.department)
  );

  return (
    <div className="min-h-screen py-20 px-4 bg-background motion-safe:animate-fade-in">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-16">
          ...
          <h1 className="font-display text-5xl sm:text-6xl font-extrabold text-foreground mb-4 tracking-tight">
            {teamConfig.title}
          </h1>
        ...
        {grouped.map((dept) => (
          <section key={dept.key} aria-labelledby={`dept-${dept.key}`} className="mb-14">
            <SectionDivider id={`dept-${dept.key}`} label={dept.label} color={dept.color} />

            {dept.key === "บริหาร" ? (
              // Leadership: dark surface grid — highest visual weight
              ...
              <div className="space-y-3">
                {/* Head: tinted surface — mid visual weight */}
                {dept.head && (
                  <DeptHeadCard member={dept.head} deptColor={dept.color} />
                )}
                {/* Members: minimal grid — lowest visual weight */}
                {dept.members.length > 0 && (
                  <div className={`grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3${dept.head ? " mt-4" : ""}`}>
                    {dept.members.map((m) => (
        ...
        {/* Unassigned members */}
        {unassigned.length > 0 && (
          <section aria-labelledby="dept-unassigned" className="mb-14">
            <SectionDivider id="dept-unassigned" label="สมาชิกอื่นๆ" color="#64748b" />
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {unassigned.map((m) => (
                <MemberCard key={m._id} member={m} />
              ))}
